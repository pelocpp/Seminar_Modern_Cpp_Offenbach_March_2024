...
namespace VariadicTemplatesIntro_Seminar {
    ...
    // spcialization for T = int
    template <>
    void printer<int>(int n) {

        std::cout << n << std::endl;
    }

    template <typename T, typename ... U>  // pack, to box , einpacken
    void printer(T n, U ... m) {           // pack, boxing

        std::cout << n << std::endl;
        printer<U ...>(m ...);                    // unpack, unboxing // auspacken
    }
    ...
    //    if constexpr( sizeof... (m) > 0 ) { // if compile time
    //        printer<U ...>(m ...);
    //    }                   // unpack, unboxing // auspacken
    //}
    ...
    //std::unique_ptr<T> myMakeUnique(TArgs& ... args)
    //{
